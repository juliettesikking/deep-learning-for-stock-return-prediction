...
class FinPiInputLinearReLU(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "bias_alpha", torch.tensor(bias_alpha, dtype=torch.get_default_dtype())
        )
        self.layernorm = layernorm
        self.device = device
        self.dtype = torch.get_default_dtype()

        self.weight = FinPiParameter(
            torch.zeros((n_out, n_in), device=self.device, dtype=self.dtype)
        )
        if bias_alpha:
            self.bias = FinPiParameter(
                torch.zeros(n_out, device=self.device, dtype=self.dtype)
            )
        else:
    ...
    def initialize(self, inf_layer):
        ...
        if self.bias is not None:
            inf_bias = (
                torch.randn(self.r, device=self.device)
                if inf_layer is None
                else inf_layer.bias
            )
    ...
    def __init__(
        ...
        if bias_alpha:
            ...
            self.bias = nn.Parameter(
                torch.zeros(r_out, device=device, dtype=torch.float32)
            )
        else:
            self.register_parameter("bias", None)
...
class FinPiLinearReLU(nn.Module):
    def __init__(
        ...
        self.weight = param_type(
            torch.zeros((n_out, n_in), device=self.device, dtype=self.dtype)
        )
        if bias_alpha:
            self.bias = param_type(
                torch.zeros(n_out, device=self.device, dtype=self.dtype)
            )
        else:
            self.register_parameter("bias", None)
    ...
    def initialize(self, inf_layer, prev_omega=None):
        ...
        if inf_layer is not None:
            ...
            A = torch.randn(
                self.r, self.n_out if self.output_layer else self.r, device=self.device
            )
        ...
        if self.output_layer:
            self.weight[:] = (
                self.n_out**-0.5 * (A.T) @ (self.nonlin(prev_omega @ B.T)).T
            )
        else:
            self.weight[:] = (
                self.n_out**-1.0
...
class InfPiLinearReLU(nn.Module):
    def __init__(
        ...
        self.output_layer = output_layer
        self.register_buffer(
            "layer_alpha", torch.tensor(layer_alpha, dtype=torch.get_default_dtype())
        )
        self.layernorm = layernorm
        self.device = device
        self.InfPiLinearReLUFunction = InfPiLinearReLUFunctionBuilder(
            layernorm=layernorm,
        ...
        self.register_parameter(
            name="A",
            param=InfPiParameter(
                A, apply_lr=False, requires_grad=False, optim_mode=optim_mode
            ),
        )
        self.register_parameter(
            name="Amult",
        ...
        self.register_parameter(
            name="B",
            param=InfPiParameter(
                B, apply_lr=False, requires_grad=False, optim_mode=optim_mode
            ),
        )
        ...
        if bias_alpha is not None:
            self.bias = nn.Parameter(
                torch.zeros(r_out, device=self.device, dtype=torch.float32)
            )
        else:
            self.register_parameter("bias", None)
    ...
    def initialize(self):
        ...
        self.A.normal_()
        self.B.normal_()
        self.B[:] = torch.nn.functional.normalize(self.B.float(), dim=1).to(
            self.B.dtype
        )
    ...
    def forward(self, g_in, gbar_in=None, s_in=None):
        ...
        if self.A.shape[0] != self.B.shape[0]:
            raise ValueError(
                "A and B have different sizes for M. Check that the gradient is applied to both."
            )
        ...
        if self.return_hidden:
            g_out, gbar_in, q_in, s_in = self.InfPiLinearReLUFunction.apply(
                g_in,
                self.A,
                self.Amult,
                self.B,
            ...
            g_out = self.InfPiLinearReLUFunction.apply(
                g_in,
                self.A,
                self.Amult,
                self.B,
