...
# Import PiLimit components
from pilimit_lib.inf.optim import pi_2init, PiSGD #pi_init is in utils.py
...
class DeepNetwork(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.layer3 = torch.nn.Linear(hid den_dim, hidden_dim, bias=True)
        self.output_layer = torch.nn.Linear(hidden_dim, 1, bias=False)
        self.non_linearity = torch.nn.ReLU()

        # Initialize weights using Pi initialization
    ...
    def initialize_weights(self):
        for p in self.parameters():
            if p.dim() > 1:
                pi_init(p)  # Use Pi initialization
...
def run_experiment(hidden_dim, data, feature_cols, window_size):
    ...
    for lr in np.arange(-18, -9, step=1):
        ...
        optimizer = PiSGD(model.parameters(), lr=2.0 ** lr)  # Use Pi-SGD optimizer
        criterion = MSRR()
