...
class ShallowNetwork(torch.nn.Module):
    def __init__(self, input_dim, hidden_input, hidden_output):
        super(ShallowNetwork, self).__init__()
        self.input_layer = torch.nn.Linear(input_dim, hidden_input, bias=True)
        self.hidden_layer = torch.nn.Linear(hidden_input, hidden_output, bias=True)
        self.output_layer = torch.nn.Linear(hidden_output, 1, bias=False)
...
def run_experiment(hidden_layer, data, feature_cols, window_size):
    sharpe_ratios = []
    learning_rates = []
